...
const auto_reply = async (msg) => {
  ...
  console.log("current replies", _.entries(replies))
  if (
    _.some(_.keys(replies), (k) => msg.data.msg.toLowerCase().includes(k)) &&
    _.find(_.keys(replies), (k) => msg.data.msg.includes(k))
  ) {
    ...
    send_message(
      msg.shit,
      replies[_.find(_.keys(replies), (k, v) => msg.data.msg.includes(k))],
      true
    )
  }
}
...
const prev_messages = Object.assign(
  _.map(_.keys(channels), (k) => ({ [k]: "" }))
)
...
const large_message_send = (msg, text) => {
  _.forEach(_.chunk(text.split(""), 1000), (m) =>
    send_message(msg, m.join("").replace(",", ""))
  )
}
const data = ["", "", "", "", ""]
let i = 0
...
bot.on("message", (msg) => {
  try {
    ...
    if (
      message.data.msg == prev_messages[msg.channel.name] ||
      message.data.isBot ||
      msg.author.username == "Dedo#2603"
    )
      return
    if (message.data.msg == "reset") {
      replies = {}
      send_message(msg, "dedo is RESET")
    }
    ...
    if (message.data.msg == "dedo take a shit") {
      _.forEach(
        _.chunk(
        ...
        (m) => send_message(msg, m.join("").replace(",", ""))
      )
    }
    if (message.data.msg == "dedo take a better shit") {
      _.forEach(
        _.chunk(
          _.map(
            data,
            (d, j) => `message number ${j + 1}: \n\n  ${util.inspect(d)}\n\n`
          )
            .join("")
            .split(""),
          1000
    ...
    if (message.data.msg == "shuttup dedo") {
      dedo_status.active = false
      send_message(msg, "youre a big bum", true)
    }
    if (message.data.msg == "dedo come back") {
      dedo_status.active = true
      send_message(msg, "im back, wheres my wine")
    }
    if (!dedo_status.active) return

    if (message.data.msg == "dedo get fancy") {
      dedo_status.fancy_text = true
      send_message(msg, "check me outttt")
    }
    if (message.data.msg == "dedo stop being a girl") {
      dedo_status.fancy_text = false
      send_message(msg, "shuttup mary")
    }
